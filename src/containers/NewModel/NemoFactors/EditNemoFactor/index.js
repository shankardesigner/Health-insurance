...
export default function EditNemoFactor(props) {
    ...
    const { openModal, setModalOpen, data, setSelectedRowData } = props;
    ...
    const defaultSelectedIntensity = data.intensity.charAt(0).toUpperCase() + data.intensity.slice(1);
    ...
    const [previouslySelectedIntensity, setPreviousIntensity] = useState(nemoFactorIntensity);
    ...
    const handleModalApply = () => {
        if(saveServiceCategoryData.length > 0){
            ...
            const newUsedNemoFactorState = {...usedNemoFactor, [nemoFactorId]:{...data, intensity:nemoFactorIntensity, isSelected: true}};
    ...
    const handleNemoFactorChange = (inputValue, id) => {
        ...
        const originalData = parseFloat(Number(serviceCategoryData[indexToUpdate].nemoFactor)).toFixed(2);
        ...
        if(originalData != newData){
            ...
            setIsCustom(true);
            temporaryData[indexToUpdate] = {...temporaryData[indexToUpdate], intensity: 'Custom'};

            setChangedServiceCategories({...changedServiceCategories, [temporaryData[indexToUpdate].nemoFactor]: temporaryData[indexToUpdate]})
        }else{
            let tempChangedServiceCategories = JSON.parse(JSON.stringify(changedServiceCategories));
            delete tempChangedServiceCategories[temporaryData[indexToUpdate].nemoFactor];
            ...
            <DialogContent>
                <BoxWithToggle title="NEMO Factor Intensity" hideToggle={true}>
                    <RadioGroup aria-label="quiz" name="quiz" value={nemoFactorIntensity} onChange={handleNemoFactorIntensityChange} className={styles.directionRow}>
                        ...
                        <FormControlLabel value="Custom" control={<Radio />} label="Custom" disabled/>
                    </RadioGroup>
                </BoxWithToggle>
